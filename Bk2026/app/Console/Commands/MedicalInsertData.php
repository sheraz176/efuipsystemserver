...
class MedicalInsertData extends Command
{
    /**
    ...
    {
        $filePath = storage_path('app/jazz/medical.csv');

        if (!file_exists($filePath)) {
            Log::channel('sms_api')->warning("Jazz medical CSV file not found: {$filePath}");
            return;
        }
...
if ($planId == 5) {
    ...
    if ($plan && $productCheck) {
        ...
        foreach ($smsList as $index => $message) {
            ...
            if ($error) {
                Log::channel('sms_api')->error("JazzCash medical SMS send error", [
                    'MobileNo' => $subscriber_msisdn,
                    'sms'      => $message,
                    'error'    => $error
                ]);
            } else {
                Log::channel('sms_api')->info("JazzCash Medical SMS API Response", [
                    'MobileNo' => $subscriber_msisdn,
                    'sms'      => $message,
                    'response' => $response
                ]);
        ...
        $newPath = storage_path('app/jazz/complete/jazzmedical_' . now()->format('Ymd_His') . '.csv');
        rename($filePath, $newPath);

        Log::channel('sms_api')->info("Jazz medical CSV file processed and moved to complete: {$newPath}");
    }
}
