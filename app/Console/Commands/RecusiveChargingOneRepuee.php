...
class RecusiveChargingOneRepuee extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'recusive:onerepuee';
   ...
   public function handle()
    {
        ...
        $targetDate = Carbon::now()->subDays(30)->toDateString(); // 30 din pehle ki date

        // Query subscriptions with recursive charging date due today and policy_status = 1

        $subscriptions =  CustomerSubscription::where('transaction_amount', 1)
        ->where('plan_id', 4)
        ->where('productId', 8)
        ...
        foreach ($subscriptions as $subscription) {
            ...
            if (isset($response['data'])) {
                ...
                if ($data !== null && isset($data['resultCode']) && $data['resultCode'] === "0") {
                    ...
                    $nextChargingDate = Carbon::tomorrow()->toDateString(); // Aaj se ek din aage
