...
class MainRecusive extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'main:recusive';
     ...
     * @var string
     */
    protected $description = 'Main Recusive Command to handle recusive charging tasks';
    ...
    public function handle()
    {
        ...
        foreach ($subscriptions as $subscription) {
            ...
            $msisdn = $subscription->subscriber_msisdn;
            if ($subscription->transaction_amount == 2) {
                $amount = 299;
            } elseif ($subscription->transaction_amount == 1) {
                if ($subscription->product_duration == 30) {
                    $amount = 299;
                } else {
                    $amount = 12;
                }
            } elseif ($subscription->transaction_amount == 163) {
                $amount = 199;
            } else {
            ...
            Log::channel('MainRecusive')->info('Recusive Charging Api.', [
                'url' => $url,
                'request-packet' => $body,
            ...
            if (isset($response['data'])) {
                ...
                if ($data !== null && isset($data['resultCode']) && $data['resultCode'] === "0") {
                    ...
                    // dd($data);
                    // Increment
                    $newCount = DB::table('customer_subscriptions')
                        ->where('subscription_id', $subscription->subscription_id)
                    ...
                    $recusive_charging_data   = new Recusivefailed();
                    $recusive_charging_data->subscription_id = $subscription->subscription_id;
                    $recusive_charging_data->tid = !empty($data['transactionId']) ? $data['transactionId'] : null;
                    $recusive_charging_data->reference_id = !empty($data['referenceId']) ? $data['referenceId'] : null;
                    $recusive_charging_data->amount = !empty($data['amount']) ? $data['amount'] : null;
