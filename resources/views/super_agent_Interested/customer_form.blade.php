        ...
        <div class="row mt-3" id="customerDataSection" style="display: none;">
            <div class="col-md-12">
                ...
                <form id="customerDataForm">
                    ...
                    <input type="hidden" class="form-control" id="plan_id" name="plan_id" readonly>
                    <input type="hidden" class="form-control" id="product_id" name="product_id" readonly>
                    <input type="hidden" class="form-control" id="product_amount" name="product_amount" readonly>
                    ...
                    <button id="autoInterested" class="btn btn-success" type="button">Proceed to Interested</button>
...
<div class="modal fade" id="successModalInterested" data-bs-backdrop="static" tabindex="-1">
    <div class="modal-dialog modal-dialog-centered" style="max-width: 30%;">
      <form class="modal-content">
        <div class="modal-header justify-content-center">
          <h5 class="modal-title">
            <i class="bi bi-check-circle" style="font-size: 2rem; color: #28a745; vertical-align: middle;"></i>
             Interested Customer successfully Registered
          </h5>
          <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
        </div>
        <div class="modal-body mt-3 text-center" id="successModalBody" style="font-size: 1.25rem;"> <!-- Adjust text size here -->
  ...
  <div class="modal fade" id="failedInterestedModal" data-bs-backdrop="static" tabindex="-1">
    <div class="modal-dialog modal-dialog-centered" style="max-width: 30%;">
      <form class="modal-content">
        <div class="modal-header justify-content-center">
          <h5 class="modal-title">
        ...
        <div class="modal-body mt-3 text-center" id="Interestedbody" style="font-size: 1.25rem;"> <!-- Adjust text size here -->
          <!-- Modal body content goes here -->
        </div>
        <div class="modal-footer text-center">
          <button type="button" class="btn btn-primary" data-bs-dismiss="modal" onclick="resetForm()">Proceed to Next Deduction</button>
    ...
    function enableAutoDebitButton() {
        ...
        $(document).ready(function () {
            $('#customerSearchForm').submit(function (event) {
                ...
                $.ajax({
                    type: 'POST',
                    url: '{{ route("super_agent_interested.fetch_customer_data") }}',
                    data: formData,
...
// Interested Customer Data

$(document).ready(function() {
    $('#autoInterested').click(function() {
        // Get the values from form fields
        var customer_msisdn = $('#customerMsisdn').val();
        var customer_cnic = $('#customerCnic').val();
        var beneficiary_msisdn = $('#beneficiaryMsisdn').val();
        var beneficiary_cnic = $('#beneficiaryCnic').val();
        var beneficiary_name = $('#beneficiaryName').val();
        ...
        $.ajax({
            type: 'POST',
            url: '{{ route("super_agent_interested.interested_customer_data") }}',
            data: requestData,
            headers: {
            ...
            success: function(response) {
                ...
                // Display success modal with response data
                $('#successModalBody').html(response.data.message);
                $('#successModalInterested').modal('show');
            },
            error: function(xhr, textStatus, errorThrown) {
                // Handle error response
                $('#customerDataForm')[0].reset(); // Reset the form
                if (xhr.status === 422) {
                    // Display failed modal with error message
                    $('#Interestedbody').html(xhr.responseJSON.data.message);
                    $('#failedInterestedModal').modal('show');
                } else {
                    // Display error modal with error message
                    $('#errorModal').modal('show');
                }
